...
10. Enforce global IP uniqueness by merging candidate IPLists that share at least one IP. The merged target keeps one deterministic name (prefer an existing DNA_* name when present, otherwise alphabetical), and aggregates all source FQDNs/IPs.
11. Create:
...
16. Build report sections:
   ...
   - IPList merge(s) performed to enforce global uniqueness.
...
- Global uniqueness enforcement merges overlapping candidates so one IP appears in only one `DNA_*` IPList at the end of each run.
