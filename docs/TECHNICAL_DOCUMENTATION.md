...
12. When updating each DNA_* IPList, keep a candidate IP if either:
...
13. Import create/update CSVs using `workloader_ipl_import.sh`.
14. Build report sections:
...
15. Send report by email using SMTP settings from `global.conf`.
